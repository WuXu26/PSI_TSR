...
df=pd.read_csv('sample_details_et_mix5_name_id_atom.csv')
...
def Cal_Centroid(res,Atoms):
    SumX = 0
    SumY = 0
    SumZ = 0
    counter = 0
    for atom1 in residue:
        if atom1.get_name() in Atoms:
...
Dict_drug={}
Output_Folder_Path="/ddnB/work/wxx6941/TSR/code/code/psi_revision/5pdb/output_chl_atom_selection_usr"
...
for i in range(len(PDB_list)):
    ...
    drug_Id = Drug_id[i]
    group=Group[i]
    atoms=Atom_list[i].split(";")
    Drug=f'{PDB_ID}_{Chain_Name}_{drug_Name}_{drug_Id}_{group}'
    #print(PDB_ID,Chain_Name,drug_Name,drug_Id)
    PDB_File_Path = "/ddnB/work/wxx6941/TSR/code/code/psi_revision/5pdb/PDB_datadir_HRemoved/{}.pdb".format(PDB_ID)
    p = Bio.PDB.PDBParser()
        ...
        if chain.id == Chain_Name:
            ...
            for residue in chain:
                ...
                if drug_Identifier == 'H' and resName == drug_Name and Res_Id == str(drug_Id):
                    Centroid_X,Centroid_Y,Centroid_Z=Cal_Centroid(residue,atoms)
                    NA_Centroid_X,NA_Centroid_Y,NA_Centroid_Z,DA_Centroid_X,DA_Centroid_Y,DA_Centroid_Z,CM,CV,CS=Cal_NA_Centroid(Centroid_X,Centroid_Y,Centroid_Z,residue,atoms)

                    DPA_Centroid_X,DPA_Centroid_Y,DPA_Centroid_Z,DAM,DAV,DAS=Cal_DPA_Centroid(DA_Centroid_X,DA_Centroid_Y,DA_Centroid_Z,residue,atoms)

                    NAM, NAV, NAS = Cal_MVS(NA_Centroid_X,NA_Centroid_Y,NA_Centroid_Z, residue,atoms)
                    DPAM, DPAV, DPAS = Cal_MVS(DPA_Centroid_X,DPA_Centroid_Y,DPA_Centroid_Z, residue,atoms)
